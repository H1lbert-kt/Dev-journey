...
@app.get("/_test-fix")
async def _test_fix(token: str = ""):
    """Testa o fluxo completo: envia alerta Telegram com botoes de aprovacao."""
    if token != _TEST_SECRET:
        return JSONResponse(status_code=404, content={"error": "not found"})
    ...
    send_error_alert(
        ...
        error_value="Erro de teste do fluxo de correcao via Telegram. Clique em 'Sim, corrigir' para testar.",
        endpoint="/_test-fix",
        method="GET",
        environment="test",
        timestamp=time.strftime("%Y-%m-%d %H:%M:%S"),
        frame_info="main.py:_test_fix",
    ...
    return JSONResponse(content={
        "success": True,
        "approval_id": approval_id,
        "message": "Alerta enviado ao Telegram. Acesse o Telegram para aprovar/rejeitar a correcao.",
        "url": f"/_test-fix-approve?token={_TEST_SECRET}&approval_id={approval_id}",
    })


@app.get("/_test-fix-approve")
async def _test_fix_approve(token: str = "", approval_id: str = ""):
    """Aprova automaticamente uma correcao de teste (para fluxo sem callback)."""
    if token != _TEST_SECRET:
        return JSONResponse(status_code=404, content={"error": "not found"})

    return JSONResponse(content={
        "approval_id": approval_id,
        "message": "Use os botoes no Telegram para aprovar. Este endpoint e apenas para verificacao.",
    })


@app.get("/_test-deploy")
async def _test_deploy(token: str = ""):
    """Testa o fluxo de deploy: envia resultado simulado de correcao."""
    if token != _TEST_SECRET:
        return JSONResponse(status_code=404, content={"error": "not found"})

    from app.notifications.telegram import send_fix_result
    import secrets
    ...
    send_fix_result(
        approval_id=approval_id,
        error_type="TestError",
        error_value="Erro simulado para teste de deploy",
        files=["app/test_file.py"],
        summary="Correcao de teste: arquivo corrigido com sucesso.",
        tests_passed=True,
        tests_output="test_example: PASSED\n1 passed in 0.01s",
    )

    return JSONResponse(content={
        "success": True,
        "approval_id": approval_id,
        "message": "Resultado de correcao enviado ao Telegram com botoes de deploy.",
    })
# ============================================================
